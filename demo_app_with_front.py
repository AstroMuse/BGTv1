...
async def search_papers(request: SearchRequest):
    ...
    try:
        ...
        if request.google_serper_key:
            ...
            _k = request.google_serper_key
            logger.info(f"Google Serper Key set from request: {_k[:4]}...{_k[-4:]}")
...
async def _simple_search(request: SearchRequest) -> SearchResponse:
    ...
    try:
        ...
        # 执行搜索
        query_results, all_papers, query_source_map, query_keywords2raw = multi_search_agent.search(
