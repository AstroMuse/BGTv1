...
# ----------------------------------------------------------------------------
# Intent-alignment scoring (default fine-rank prompt, SPAR_SCORING_PROMPT=intent)
# Replaces generic relevance with "paper core intent vs query search intent".
# ----------------------------------------------------------------------------
template_sim_intent_align_inst = """### **Task:**
You are ranking candidate papers for an academic paper-search engine. Judge how well the paper's **core research intent** aligns with the **search intent of the query** — NOT merely whether they share keywords, techniques, or a research domain.
...
### **Step 1 — Identify both intents (write one short sentence each):**
- **query_intent:** what kind of paper is the user actually looking for? Identify the *task/problem*, the *method/technique*, the *domain*, and the **role** the method plays in the request.
- **paper_core_intent:** the paper's central contribution — what the paper is fundamentally *about*. Distinguish this from techniques the paper merely uses as a tool, improves in passing, treats as a baseline, or evaluates on.

### **Step 2 — Score (0-5 each):**
1. **intent_alignment:** Is the paper's central contribution exactly the kind of work the query asks for?
   - 5 = the paper's main subject is precisely the queried task+method combination.
   - <=2 = the queried concept appears only as a tool, background, baseline, or serves a different goal.
   - Calibration example: for the query "prior studies using GANs for text-to-image generation", a paper whose contribution is *a more efficient GAN architecture* (evaluated on generic image benchmarks) only **uses** GANs — its core intent is architecture efficiency, not text-to-image generation, so intent_alignment <= 2 despite full keyword overlap.
   - Exception (overrides the rule above): if the paper is a seminal, foundational, review, or survey paper **whose own subject is the queried method or task** (e.g. the original GAN paper, or a survey of GANs, for a query about studies using GANs), set intent_alignment = 3 — researchers issuing this query would legitimately cite it even though it is not a direct answer. This exception does NOT apply to papers that merely use or modify the method for a different goal.
2. **task_scope_match:** Does the paper's concrete task/domain coincide with the query's? (e.g. text-to-image generation != unconditional image generation; medical NER != general NER.)
3. **evidence_directness:** Would a researcher who issued this query cite this paper as a **direct answer** to it, rather than as related background reading?
...
```json
{
  "query_intent": "<one short sentence>",
  "paper_core_intent": "<one short sentence>",
  "intent_alignment": X,
  "task_scope_match": X,
...
Ensure the response strictly follows the given JSON format for easy parsing. Do not include any extra text outside of this format.

**Output:**"""


# ---------------------------------------------------------------------------
# Batched version of the intent-alignment scorer: score MANY documents against
# the same query in ONE call. query_intent is stated once; each document gets
# its own paper_core_intent + 3 scores. Output is index-anchored ([DOC i] ->
# "idx": i) so the caller can map results back even if the model reorders or
# drops entries. {documents} is built by the caller (one [DOC i] block per doc).
# ---------------------------------------------------------------------------
template_sim_intent_align_batch_inst = """### **Task:**
You are ranking candidate papers for an academic paper-search engine. For EACH document below, judge how well the paper's **core research intent** aligns with the **search intent of the query** — NOT merely whether they share keywords, techniques, or a research domain.

---

### **Query (applies to every document):**
- **Search Time:** {searchTime}
- **UserQuery:** {userQuery}
...
### **Step 1 — Identify the query intent ONCE:**
- **query_intent:** what kind of paper is the user actually looking for? Identify the *task/problem*, the *method/technique*, the *domain*, and the **role** the method plays in the request.

### **Step 2 — For EACH document, write its core intent and score it (0-5 each):**
- **paper_core_intent:** the paper's central contribution — what the paper is fundamentally *about*. Distinguish this from techniques the paper merely uses as a tool, improves in passing, treats as a baseline, or evaluates on.
1. **intent_alignment:** Is the paper's central contribution exactly the kind of work the query asks for?
   - 5 = the paper's main subject is precisely the queried task+method combination.
   - <=2 = the queried concept appears only as a tool, background, baseline, or serves a different goal (e.g. a paper whose contribution is a more efficient GAN architecture only *uses* GANs — core intent is architecture efficiency, not the queried task).
   - Exception (overrides the rule above): a seminal, foundational, review, or survey paper **whose own subject is the queried method or task** → set intent_alignment = 3 (researchers issuing this query would legitimately cite it). Does NOT apply to papers that merely use/modify the method for a different goal.
2. **task_scope_match:** Does the paper's concrete task/domain coincide with the query's? (e.g. text-to-image generation != unconditional image generation; medical NER != general NER.)
3. **evidence_directness:** Would a researcher who issued this query cite this paper as a **direct answer**, rather than as related background reading?
---
"""

template_sim_intent_align_batch_example = """
### **Output Format (Structured JSON — ONE object):**
```json
{
  "query_intent": "<one short sentence>",
  "papers": [
    {"idx": 0, "paper_core_intent": "<one short sentence>", "intent_alignment": X, "task_scope_match": X, "evidence_directness": X},
    {"idx": 1, "paper_core_intent": "<one short sentence>", "intent_alignment": X, "task_scope_match": X, "evidence_directness": X}
  ]
}
```

Score EVERY document. The "papers" array MUST contain exactly one entry per document, each with the matching "idx" (the [DOC i] number shown above). Output strictly this JSON object and nothing else.

**Output:**"""


# ===========================================================================
# Agent B — LLM "memory" recall + existence verification
# ===========================================================================
template_agentb_recall = """You are an expert research librarian with broad first-hand knowledge of the published academic literature. A user is searching for papers.
...
### Task:
From your OWN knowledge of the literature, recommend the {recall_num} papers that BEST match the user's query. For each paper give its exact title and, if you know it, its arXiv identifier (e.g. "2107.15245" or "hep-ph/9606011").

Rules:
- Only include papers you are genuinely confident EXIST. Returning fewer real papers is far better than inventing plausible-sounding ones.
- If you are unsure of the arXiv id, leave "arxiv_id" as "" rather than guessing a fabricated number.
- Respect the time constraint above (recommend nothing published outside it).

### Output (strict JSON, nothing else):
```json
{{"papers": [{{"title": "<exact title>", "arxiv_id": "<id or empty>"}}]}}
```
**Output:**"""


template_agentb_verify = """You previously recommended candidate papers for a user query. Each candidate's arXiv id / title was looked up in a local academic database and on web search to check whether it really exists. Below is every candidate together with the evidence retrieved for it.

### User query:
{userQuery}

### Candidates with retrieved evidence:
{candidates}

### Task:
For EACH candidate decide whether it is a REAL, existing paper that genuinely matches the title/topic that was recommended — judging ONLY from the retrieved evidence.
- Set "exists": true only when the evidence (a resolved real title + abstract) clearly corresponds to the recommended paper.
- If the evidence is empty, or the resolved title is unrelated to the recommended title, set "exists": false (the id/title was likely hallucinated).

### Output (strict JSON, nothing else):
```json
{{"results": [{{"idx": 0, "exists": true}}, {{"idx": 1, "exists": false}}]}}
```
**Output:**"""


# ===========================================================================
# Agent C — multi-dimensional intent-constraint decomposition
# ===========================================================================
template_agentc_decompose = """You are an expert at analyzing academic search queries. Decompose the user's query into a structured, multi-dimensional intent-constraint specification.

### User query:
{userQuery}
### Search time (today): {searchTime}

### Dimensions — give the MINIMUM constraint each dimension actually warrants; leave a dimension empty when the query does not constrain it:
1. time_constraint: any time/date description in the query, normalized to {{"start": "YYYY-MM-DD", "end": "YYYY-MM-DD"}}. Either endpoint may be empty; BOTH empty = no time constraint.
2. domain_constraint: if the query is a natural-language question, give an appropriately-scoped domain descriptor. The papers the user needs MUST fall inside this domain, but do not make it overly broad.
3. literature_type: which of these the query asks for — "review", "original/empirical", "theory/method", "data", "case study", "opinion/position". If none of the six fit, describe in your own words the type the user wants. Empty if unconstrained.
4. keywords: keywords from the query. Keep the user's own fragments verbatim; if the query is natural language, distill them. Keyword scope = explicit journal names, author names, institutions/companies/universities, abbreviations, and recognized academic terms. Principle: meaning-accurate but spelling may vary slightly (the user may misspell).
5. extension: a free dimension. ONLY if dimensions 1-4 cannot fully and accurately capture the query's constraints, add up to TWO extra named constraints here, each as {{"name": "<name>", "value": "<value>"}}.

### Output (strict JSON, nothing else):
```json
{{"time_constraint": {{"start": "", "end": ""}}, "domain_constraint": "", "literature_type": "", "keywords": [], "extension": []}}
```
**Output:**"""


template_agentc_priority = """You are preparing an academic search. Below is the user's original query and its decomposed intent constraints.

### Original query:
{userQuery}
### Decomposed constraints (JSON):
{constraints}

### The downstream search uses these APIs: Semantic Scholar, Google (arXiv via Serper), OpenAlex.
Among these, the ONLY constraint that can be passed as a native filter parameter is the time range (a date filter). Treat EVERY other constraint as NOT directly parametrizable.

### Task:
1. Classify each non-empty constraint as HARD (must hold; high priority) or SOFT (preferred; low priority). An empty dimension imposes no constraint.
2. Emit the HARD constraints that ARE parametrizable as a flat key/value dict "params" — currently only "start_date" and "end_date" in "YYYY-MM-DD" (omit a key when there is no time bound).
3. For every HARD constraint that is NOT parametrizable, PLUS the SOFT constraints, rewrite the query into ONE fluent natural-language search question ("new_query") that weaves those constraints in naturally. Every non-parametrizable HARD constraint MUST appear in new_query; include SOFT constraints where they help.
4. Echo the original query unchanged as "source_query".

### Output (strict JSON, nothing else):
```json
{{"params": {{"start_date": "", "end_date": ""}}, "new_query": "<rewritten question>", "source_query": "<original query>"}}
```
**Output:**"""
...
# ============================================================================
# Multi-level domain tree (Idea 1): build broad, then prune to the query.
# ============================================================================
template_domain_tree = """You are an expert in organizing academic knowledge into a two-level hierarchy of research subfields.

Given a research query QUERY and a top-level domain DOMAIN, construct a 2-layer subfield tree.

Structure:
- Root node: the given DOMAIN (1 node).
- L1: exactly 3 children of the root.
- L2: exactly 3 children for each L1 node (9 leaf nodes in total).
- L2 leaf nodes must include "children": [].

Key constraints:
1. Query focus: Each node must be directly relevant to the QUERY, not only to the parent domain.
   Prune away subfields of {domain} that are irrelevant to the QUERY.
2. Sibling independence: All 3 children under the same parent must represent distinct, non-overlapping research directions.
   Do not restate or overlap sibling concepts.
3. Specificity: L2 nodes must be more specific than their L1 parent, indicating concrete methods, tasks, or application areas
   (e.g., "LoRA fine-tuning" rather than just "fine-tuning").

Output strictly only JSON (no markdown, no text, no comments):
{{"name": "<root>", "children": [{{"name": "<L1>", "children": [{{"name": "<L2>", "children": []}}]}}]}}
...
template_domain_tree_prune = ""  # pruning removed; tree is built compact from the start


# ---------------------------------------------------------------------------
# Scheme 2: domain-vocabulary-augmented query expansion
# domain_vocabulary_text is rendered by QueryAgent._format_domain_vocabulary()
# ---------------------------------------------------------------------------
template_domain_aware_query_expansion_with_vocab = """You are an expert academic search query \
generator specializing in the field of {domain}.

### TASK:
Generate {user_input_N} TECHNICAL and SPECIALIZED search queries to retrieve PRIMARY RESEARCH \
PAPERS relevant to:
"{user_query}"
...
### RELEVANT SUB-DOMAIN VOCABULARY:
The following two-level sub-domain structure has been identified as specifically relevant to this
query. Use these terms to make your queries more precise. Each L1 branch is an independent
research direction; try to draw from different branches across your {user_input_N} queries.

{domain_vocabulary_text}

### REQUIREMENTS:
1. Incorporate terminology from the vocabulary above where it fits naturally.
2. Cover DIFFERENT L1 branches across your queries to maximise retrieval diversity.
3. Use L2-level specificity (concrete methods/tasks) rather than broad L1 generalities.
4. Target EMPIRICAL STUDIES and PRIMARY RESEARCH, not surveys.
5. Keep each query CONCISE (5-15 words) and directly usable in academic search engines.
6. TIME-SENSITIVITY: if the original query mentions "recent" or "latest", target \
...
{{
  "expanded_queries": [
    {{
      "query": "Precise technical query string",
      "reason": "Targets [L1 branch] via [L2 term]",
      "technical_focus": "[L2 term used]"
    }}
  ],
  "domain_keywords": ["kw1", "kw2", "kw3"]
}}
"""
