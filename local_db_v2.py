...
class ArxivDatabase:
    ...
    def connect(self):
        ...
        # Wait (up to 60s) for the write lock instead of failing immediately with
        # "database is locked" — lets a benchmark run and s2_localize_references.py
        # (or any two writers) share recovered.db without write errors.
    ...
    # SQLite's max string length is ~1 GB (SQLITE_MAX_LENGTH). A blob this large
    # is always a bug (reference nesting), never legitimate metadata; refuse it.
    MAX_BLOB_BYTES = 50_000_000  # 50 MB hard cap; real arXiv docs are < ~100 KB

    @staticmethod
    def _slim_for_storage(data: Dict[str, Any]) -> Dict[str, Any]:
        """De-nest references before storing.

        A stored doc keeps its OWN `references` list (each ref carries
        title/abstract for Phase-2 scoring), but those ref entries must NOT carry
        their own `references`/`citations` — otherwise every cache round-trip
        nests refs-of-refs until one blob exceeds SQLite's ~1 GB string limit
        (the "Error binding parameter 1 - probably unsupported type" failure) and
        balloons memory (OOM). Stripping one level off the top-level refs drops
        the entire nested subtree.
        """
        ...
        if not isinstance(refs, list) or not refs:
            return data
        drop = ("references", "citations", "references_raw")
        slimmed = [
            {k: v for k, v in r.items() if k not in drop} if isinstance(r, dict) else r
            for r in refs
        ]
        out = dict(data)
        out["references"] = slimmed
        return out

    def update_or_insert(self, arxiv_id: str, data: Dict[str, Any]):
        """Update or insert a single record (references de-nested, size-guarded)."""
        try:
            data_json = json.dumps(self._slim_for_storage(data))
            if len(data_json) > self.MAX_BLOB_BYTES:
                logger.error(
                    f"Skip oversized record {arxiv_id}: {len(data_json)} bytes "
                    f"(> {self.MAX_BLOB_BYTES}); likely reference nesting"
                )
                return
